...
AdvancedGauge::AdvancedGauge(QWidget *parent) : QWidget(parent)
{
    ...
    connect(m_gauge, &GaugeWidget::valueChanged, this, &AdvancedGauge::onGaugeValueChanged);
    connect(m_gauge, &GaugeWidget::setpointChanged, this, &AdvancedGauge::onGaugeSetpointChanged);
...
void AdvancedGauge::onGaugeValueChanged(double v)
{
    m_currentLabel->setText(QString::number(v, 'f', 1) + "°");
    emit valueChanged(v);
}

void AdvancedGauge::onGaugeSetpointChanged(double sp)
{
    m_setpointLabel->setText("Set: " + QString::number(sp, 'f', 1) + "°");
    emit setpointChanged(sp);
}
